...
# Define the body
radius, center = ly, (lx/2, ly/2) .+ (0.01, 0.01)
...
uₒx, uₒy = (ones((nx+1)*(ny+1)), ones((nx+1)*(ny+1))) #.* (cos(pi/4), sin(pi/4))
uγx, uγy = (ones((nx+1)*(ny+1)), ones((nx+1)*(ny+1))) #.* (cos(pi/4), sin(pi/4))
...
Fluide = Phase(capacity, operator, f, 0.25)
