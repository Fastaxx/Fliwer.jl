...
function VOFI(body::AbstractBody, mesh::AbstractMesh)
    ...
    Vs, bary, interface_length, cell_types = CartesianGeometry.integrate(Tuple{0}, body.sdf, mesh.nodes, Float64, zero)
    As = CartesianGeometry.integrate(Tuple{1}, body.sdf, mesh.nodes, Float64, zero)
    Ws = CartesianGeometry.integrate(Tuple{0}, body.sdf, mesh.nodes, Float64, zero, bary)
